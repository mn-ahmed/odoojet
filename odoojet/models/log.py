...
class odoojetLog(models.Model):
    _name = "odoojet.log"
    _description = "odoojet - Log"
    ...
    line_ids = fields.One2many("odoojet.log.line", "log_id", string="Fields updated")
    http_session_id = fields.Many2one(
        "odoojet.http.session", string="Session", index=True
    )
    http_request_id = fields.Many2one(
        "odoojet.http.request", string="HTTP Request", index=True
    )
...
class odoojetLogLine(models.Model):
    _name = "odoojet.log.line"
    _description = "odoojet - Log details (fields updated)"

    field_id = fields.Many2one(
        "ir.model.fields", ondelete="set null", string="Field", index=True
    )
    log_id = fields.Many2one(
        "odoojet.log", string="Log", ondelete="cascade", index=True
    )
