...
class odoojetHTTPRequest(models.Model):
    _name = "odoojet.http.request"
    _description = "odoojet - HTTP request log"
    ...
    http_session_id = fields.Many2one(
        "odoojet.http.session", string="Session", index=True
    )
    user_context = fields.Char("Context")
    log_ids = fields.One2many("odoojet.log", "http_request_id", string="Logs")
    ...
    def current_http_request(self):
        ...
        if httprequest:
            if hasattr(httprequest, "odoojet_http_request_id"):
                # Verify existence. Could have been rolled back after a
                # concurrency error
                self.env.cr.execute(
                    "SELECT id FROM %s WHERE id = %s",
                    (AsIs(self._table), httprequest.odoojet_http_request_id),
                )
                if self.env.cr.fetchone():
                    return httprequest.odoojet_http_request_id
            vals = {
            ...
            httprequest.odoojet_http_request_id = self.create(vals).id
            return httprequest.odoojet_http_request_id
        return False
