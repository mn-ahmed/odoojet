...
class odoojettHTTPSession(models.Model):
    _name = "odoojet.http.session"
    _description = "odoojet - HTTP User session log"
    ...
    http_request_ids = fields.One2many(
        "odoojet.http.request", "http_session_id", string="HTTP Requests"
    )
    ...
    def current_http_session(self):
        ...
        if httpsession:
            ...
            httpsession.odoojet_http_session_id = self.create(vals).id
            return httpsession.odoojet_http_session_id
        return False
